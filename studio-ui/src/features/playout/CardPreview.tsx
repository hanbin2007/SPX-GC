...
import { groupMembers, SCHOOL_EMBLEM_URL, slotField } from '@/domain/logos';
...
function LogoStrip({ values }: { values: Record<string, string> }) {
  ...
  const members = follow === 'school' ? [0] : groupMembers(values, follow);
  const urls = (members.length ? members : [0]).map((index) => index === 0
    ? SCHOOL_EMBLEM_URL
    : assets.find((asset) => asset.value === values[slotField.file(index)])?.url);
  return (
...
export function CardPreview({ view }: { view: ItemView }) {
  const lines = previewLines(view.item, view.values);
  return (
